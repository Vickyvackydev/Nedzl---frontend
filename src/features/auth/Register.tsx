...
function Register() {
  ...
  formInput.email === '' || !passwordPattern.test(formInput.password) || formInput.phone_number === '';
  ...
  return (
    <>
      <SEO title="Register" description="Create a new Nedzl account." noindex={true} />
      <AuthLayout>
        <motion.div
        ...
        >
          <div className="flex flex-col items-center justify-center gap-y-2">
            ...
            <span className="text-2xl font-bold text-primary-300">Create account</span>
            <span className="text-primary-300 font-normal text-sm">
              Already have an account?{' '}
              <span className="text-global-green font-medium cursor-pointer" onClick={() => navigate('/login')}>
                Login
              </span>
            </span>
          </div>
          <form action="" onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-y-4">
            <div className="flex lg:flex-row flex-col gap-y-3 items-start w-full gap-x-4">
              <div className="flex flex-col items-start w-full gap-y-1">
                <span className="text-sm font-medium text-[#4F5762]">Username</span>
            ...
            <div className="flex flex-col items-start w-full gap-y-1">
              ...
              {formInput.email && !emailRegex.test(formInput.email) && (
                <span className="text-xs font-normal text-red-500">Kindly use a valid email address</span>
              )}
            </div>
            <div className="flex flex-col items-start w-full gap-y-1">
              <span className="text-sm font-medium text-[#4F5762]">Phone Number</span>
              <div className="w-full h-[48px] rounded-xl px-3 border border-borderColor shadow-input flex items-center gap-x-2">
              ...
              <div className="mt-3 flex items-start justify-between w-full">
                <div className="flex flex-col gap-y-2">
                  <div className="flex items-center gap-x-1">
                    {checkers[0] ? (
                      <img src={CHECK_GREEN} className="w-[16px] h-[16px]" alt="" />
                    ) : (
                      <div className="w-[16px] h-[16px] rounded-full border border-borderColor" />
                    )}

                    <span className="text-xs font-normal text-primary-300">Atleast 8 characters</span>
                  </div>
                  <div className="flex items-center gap-x-1">
                    {checkers[1] ? (
                      <img src={CHECK_GREEN} className="w-[16px] h-[16px]" alt="" />
                    ...
                    <span className="text-xs font-normal text-primary-300">Atleast one uppercase</span>
                  </div>
                  <div className="flex items-center gap-x-1">
                    {checkers[2] ? (
                      <img src={CHECK_GREEN} className="w-[16px] h-[16px]" alt="" />
                    ...
                    <span className="text-xs font-normal text-primary-300">Atleast one number</span>
                  </div>
                </div>
                <div className="flex items-center gap-x-1">
                  {checkers[3] ? (
                  ...
                  <span className="text-xs font-normal text-primary-300">Atleast one special character (!@#$%)</span>
                </div>
              </div>
            ...
            <div className="flex flex-col items-start w-full gap-y-1">
              <span className="text-sm font-medium text-[#4F5762]">Confirm Password</span>
              <div className="w-full h-[48px] justify-between rounded-xl px-3 border border-borderColor shadow-input flex items-center gap-x-2">
                <div className="flex items-center gap-x-2">
                  <img src={PAD_LOCK} className="w-[20px] h-[20px]" alt="" />
                ...
                {showConfirmPassword ? (
                  <span className="cursor-pointer " onClick={() => setShowConfirmPassword(false)}>
                    <FaEyeSlash color="#808080" />
                  </span>
                ) : (
                  <span className="cursor-pointer" onClick={() => setShowConfirmPassword(true)}>
                    <FaEye color="#808080" />
                  </span>
                )}
              </div>

              {formInput.confirm_password && formInput.confirm_password !== formInput.password && (
                <span className="text-xs font-normal text-red-500">Password must match</span>
              )}
            </div>
            <div className="flex items-center gap-x-3" onClick={() => setChecked((prev) => !prev)}>
              ...
              <span className=" lg:text-sm text-xs font-normal text-primary-50">
                ...
                <span onClick={() => navigate('/terms-of-service')} className="text-global-green cursor-pointer">
                  Terms of service
                </span>{' '}
                and{' '}
                <span onClick={() => navigate('/privacy-policy')} className="text-global-green cursor-pointer">
                  Privacy policy
                </span>
              </span>
            </div>
